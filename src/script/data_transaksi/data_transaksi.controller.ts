...
export default class DataTransaksiController {
  public async GetData(_: Request, res: Response) {
    try {
      const result = await dataTransaksi.findMany({ include: { data_user: true, DataDetailTransaksi: { include: { data_produk: true, data_kategori: true, data_toko: true } } }, orderBy: { createdAt: 'desc' } });
  ...
  public async GetDataById(req: Request, res: Response) {
    try {
      ...
      const result = await dataTransaksi.findFirst({ where: { id: id }, orderBy: { createdAt: 'desc' }, include: { data_user: true, DataDetailTransaksi: { include: { data_produk: true, data_kategori: true, data_toko: true } } } });
  ...
  public async GetDataByIdUser(req: Request, res: Response) {
    try {
      ...
      const result = await dataTransaksi.findMany({ where: { id_user: id_user }, orderBy: { createdAt: 'desc' }, include: { data_user: true, DataDetailTransaksi: { include: { data_produk: true, data_kategori: true, data_toko: true } } } });
      res.json({ data: result, status: 'Success' });
    } catch (error) {
      res.status(500).json({ message: `${ErrorH(error)}`, status: 'Error' });
    }
  ...
  public async GetDataByIdToko(req: Request, res: Response) {
    try {
      ...
      const result = await dataTransaksi.findMany({
        where: { OR: [{ id_user: id_toko }, { DataDetailTransaksi: { every: { data_toko: { id_user: id_toko } } } }] },
